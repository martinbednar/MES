...
{
    internal class PressDataReadyHandler : NewDataReadyAbstractHandler
    {
        protected override AbstractOpcUaClient opcUaClient { get; } = PressOpcUaClient.Instance;
        protected override string RootNodeId { get; } = @"ns=3;s=""dataPressOPC""";
        ...
        protected internal override async Task SaveRecord()
        {
            ...
            dbContext.PressingProcessData.Add(new PressingProcessData()
            {
                SerialNumber = serialNumber,
                HoleNumber = await opcUaClient.ReadValueAsync(RootNodeId + @".""records""[0].""holeNumber""", new Int16()),
                Force = await opcUaClient.ReadValueAsync(RootNodeId + @".""records""[0].""finalForce""", new float()),
                Position = await opcUaClient.ReadValueAsync(RootNodeId + @".""records""[0].""finalPos""", new float()),
